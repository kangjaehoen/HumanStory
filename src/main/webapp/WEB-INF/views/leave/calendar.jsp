...
<script>
	 ...
	 $(document).ready(function() {
	        ...
	        function calculateLeaveDays1() {
	            ...
	            var leaveDays1 = 0; // 총 휴가일수
	            
	            if (startDate1 && endDate1) {
	                // 종료일에서 시작일을 뺀 후 하루를 더해서 기간을 구합니다.
	                var timeDiff1 = endDate1.getTime() - startDate1.getTime();
	                var diffDays1 = Math.ceil(timeDiff1 / (1000 * 3600 * 24)) + 1;
	                // 시작일과 종료일이 같은 경우
	                if (startDate1.getTime() === endDate1.getTime()) {
	                    // 휴가 유형에 따라 총 휴가일수를 결정합니다.
	                    if (leaveType1 === '연차' || leaveType1 === '병가' || leaveType1 === '가족돌봄휴가' || leaveType1 === '출산휴가') {
	                        leaveDays1 = 1;
	                    } else if (leaveType1 === '반차') {
	                        leaveDays1 = 0.5;
	                    } else if (leaveType1 === '반반차') {
	                        leaveDays1 = 0.25;
	                    }
	                } else { // 시작일과 종료일이 다른 경우
	                    // 반차나 반반차를 선택했을 때 경고 메시지를 표시하고 다른 값을 선택하도록 합니다.
	                    if (leaveType1 === '반차' || leaveType1 === '반반차') {
	                        alert('시작일과 종료일이 다를 때 반차나 반반차는 선택할 수 없습니다.\n다른 값을 선택해주세요.');
	                        $('#leaveType1').val('연차'); // 휴가 유형을 연차로 변경합니다.
	                        return; // 함수 종료
	                    } else {
	                        // 그 외의 경우에는 실제 기간에 맞춰 총 휴가일수를 계산합니다.
	                        leaveDays1 = diffDays1;
	                    }
	                }
	            }
	
	            // 총 휴가일수를 출력합니다.
	            $('#leaveDays1').val(leaveDays1);
	        }
...
/* 삭제, 수정, 취소 버튼을 나란히 배치하기 위한 스타일 */
							...
							<template x-for="(sDate, dateIndex) in no_of_days"
								...
								<div style="width: 14.28%; height: 120px"
									...
									<div @click="showEventModal(sDate)" x-text="sDate"
										...
										:class="{'bg-gray-800 text-white': isToday(sDate) == true, 'text-gray-700 hover:bg-gray-300': isToday(sDate) == false }">
									</div>

									<div style="height: 80px;" class="overflow-y-auto mt-1">
										<template x-for="(event, eventIndex) in events"
											:key="eventIndex">
											<div x-show="isVisible(sDate, event)"
												x-text="event.empNum  + '  [' +event.leaveType  + ']'"
			...
			<div x-show.transition.opacity="isEventModalOpen"
				...
				<div @click.stop
					...
					<div class="flex items-center mb-4">
						<form action="/leave/register" method="post">
							<div class="grid gap-4 mb-4 sm:grid-cols-2">
								...
								<div>
									<label for="leaveDays"
										class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">휴가일수</label>
					...
					<div class="text-center">
						...
						<form action="/leave/remove" method="post">
								<span class="text-xl font-bold mb-4"
									...
									:value="selectedEvent.leaveNum"></input> <br>
								...
								<h2 x-text="'종료 날짜: '+selectedEvent.eDate"
									class="text-base text-red-600 mb-4 border-2 border-gray-400 mx-10 rounded-lg"></h2>
								<h2 x-text="'휴가 일수: ' + selectedEvent.leaveDays"
									class="text-sm mb-2 border-2 border-gray-400 mx-20 rounded-lg"></h2>
			...
			<div @click.stop
				...
				<form action="/leave/modify" method="post">
					...
					<div>
						<label for="leaveDays"
							class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">휴가일수</label>
						<input type="text" name="leaveDays" id="leaveDays1"
							class="bg-gray-100 border border-gray-300 text-gray-900 text-lg rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
							readonly="readonly">
					</div>
	...
	<script>
	    function app() {
	        return {
	            showEventModal(sDate = null) {
	                this.isEventModalOpen = true;
	                this.openEventModal = true;
	                if (sDate) {
	                    this.eventsDate = new Date(this.year, this.month, sDate);
	                }
	            },
	            ...
	            isVisible(sDate, event) {
	                const eventsDate = new Date(this.year, this.month, sDate);
	                return eventsDate.toDateString() === new Date(event.sDate).toDateString();
	            },
	            isToday(date) {
