							...
							<c:forEach items="${list}" var="email">
								<tr class="border-b dark:border-gray-700">
									<td class="px-4 py-3"><input class="tdCheck"
										type="checkbox"></td>
									<th scope="row"
										class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">${email.empNum }</th>
									<td class="px-4 py-3" id="empName">${email.empName}</td>
									<td class="px-4 py-3" id="annualLeaveNum">${email.annualLeaveNum}</td>
									<td class="px-4 py-3" id="email">${email.email}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
	...
	$(function() {
		...
		$(".tdCheck").change(function() {
		    if ($(this).prop("checked")) {
		        ...
		        var annualLeaveNum = empRow.find("#annualLeaveNum").text();
		        var email = empRow.find("#email").text();

		        console.log("empName :" + empName);
		        console.log("email :" + email);
		        console.log("annualNum :" + annualLeaveNum);
		        ...
		        $('#empEmailInfo input[name="annualLeaveNum"]').remove();
